...
import { v4 } from 'uuid';
import { requestPostsByPublicID } from './requestPostsByPublicID';
import { requestPostsByUserID } from './requestPostsByUserID';

export const publishPost = createAsyncThunk<any, string, ThunkConfig<any>>(
  'Wall/publishPost',
  async (publicID, thunkAPI) => {
    ...
    console.log({ publicID, authorID, formattedDate, text });

    const post = {
      createdAt: formattedDate,
      img: 'https://textis.ru/wp-content/uploads/2015/03/28.png',
      publicID: publicID ? publicID : 'none',
      authorID: authorID,
      views: 0,
      blocks: [
        {
    ...
    try {
      ...
      publicID === ('none' || undefined)
        ? thunkAPI.dispatch(requestPostsByPublicID(publicID))
